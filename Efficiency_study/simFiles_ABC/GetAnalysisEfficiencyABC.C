...
void GetAnalysisEfficiencyABC(const char* analysisFileName, const char* histogramFileName, int nL, const char* outputFileName) {
    ...
    // --- Load the nominal ABC histogram ---
    TH1D* hist = dynamic_cast<TH1D*>(file1->Get("ABC"));
    if (!hist) {
        std::cerr << "Error: Histogram 'ABC' not found in " << histogramFileName << std::endl;
        file1->Close();
        file2->Close();
        return;
    }
    ...
    const int number_of_bins = 1800;
    double range_min = hist->GetXaxis()->GetXmin();
    double range_max = hist->GetXaxis()->GetXmax();
    TH1F* hist1 = new TH1F("hist1", "Rebinned ABC Flux", number_of_bins, range_min, range_max);

    for (int bin = 1; bin <= hist->GetNbinsX(); ++bin) {
        hist1->Fill(hist->GetBinCenter(bin), hist->GetBinContent(bin));
    }

    // --- Compute total integral of the nominal flux ---
    ...
    // --- Create histogram for detected energy ---
    const double xMin = 0.0;
    const double xMax = 12.0;
    TH1F* hist2 = new TH1F("hist2", "Detected Energy Distribution", number_of_bins, xMin, xMax);
